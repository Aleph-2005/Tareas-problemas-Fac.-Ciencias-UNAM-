...
\begin{proof}\,\\
    ...
    $\supseteq$):\,\\
    \,\\
    Como $\langle\{x\}\rangle$ es un subespacio vectorial (intersecci\'on de subespacios es subespacio) que contiene a $x$, este becesariamente debe contener a todas
...
\begin{proof}\,\\
    ...
    $\Rightarrow)$\,\\
    \,\\
    Como $W$ ya es subespacio de $V$ tenemos que $W\in\{K<F| W\subset K\} $, por tanto $\langle W \rangle\subset W$, luego
...
\begin{proof}\,\\
    ...
    \end{equation*}\,\\
    Una contradicci\'on, ya que $e^x$ no es constante, por tanto $\{e^x,e^{2x}\}$ es linealmente independiendente
\end{proof}\,\\
...
\begin{proof}\,\\
    ...
    Por inducción, el conjunto $\{e^{nx} \mid n \in \N\}$ es linealmente independiente en $\R^{\R}$, pues cualuqier subconjunto finito de este es
